...
{
    let reply_bytes = reply.to_string().as_bytes().to_vec();
    let shared_reply_bytes = Arc::new(reply_bytes);
    tokio::spawn(reply_one(first, Arc::clone(&shared_reply_bytes)));
    tokio::spawn(reply_one(second, Arc::clone(&shared_reply_bytes)));
}

pub async fn reply_one(listener: TcpListener, data: Arc<Vec<u8>>) {
    loop {
        let (mut socket, _) = listener.accept().await.unwrap();
        let shared_data = Arc::clone(&data);
        tokio::spawn(async move {
            let (_, mut writer) = socket.split();
            writer.write_all(&shared_data).await.unwrap();
        });
    }
}
